...
func BuscarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	usuarioID, erro := strconv.ParseUint(vars["usuarioId"], 10, 64)
	if erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}
...
func AtualizarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if usuarioID != usuarioIDNoToken {
		respostas.Erro(w, http.StatusForbidden, errors.New("não é possível atualizar que não seja seu"))
		return
	}
...
func SeguirUsuario(w http.ResponseWriter, r *http.Request) {
	seguidorID, erro := autenticacao.ExtrairUsuarioToken(r)
	if erro != nil {
		respostas.Erro(w, http.StatusUnauthorized, erro)
		return
	}
	...
	if seguidorID == usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("usuário não pode seguir a si mesmo"))
		return
	}
...
func DesseguirUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if seguidorID == usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("usuário não pode parar seguir a si mesmo"))
		return
	}
	...
	repositorio := repositorios.NovoRepositorioDeUsuarios(db)
	if erro = repositorio.Desseguir(usuarioID, seguidorID); erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
...
//Trás todos os usuários que um determinado usuário está seguindo
...
func Seguindo(w http.ResponseWriter, r *http.Request) {
	...
	repositorio := repositorios.NovoRepositorioDeUsuarios(db)
	seguindo, erro := repositorio.BuscarQuemSegue(usuarioID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
...
func AtualizarSenha(w http.ResponseWriter, r *http.Request) {
	...
	if usuarioIDNoToken != usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("usuario pode apenas solicitar a própria troca de senha"))
		return
	}
	...
	if erro = security.VerificarSenha(senha.Atual, senhaSalvaNoBanco); erro != nil {
